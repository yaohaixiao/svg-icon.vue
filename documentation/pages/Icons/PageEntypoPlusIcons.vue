  ...
  <base-container>
    <base-header
      ...
      <base-breadcrumb>
        ...
        <base-breadcrumb-item current>Entypo+</base-breadcrumb-item>
      </base-breadcrumb>
    ...
    <base-footer
      ...
      <div class="util-align-left base-footer__navigation">
        Prev Page：
        <router-link to="/icons/broccolidry">Eighty Shades 图标库</router-link>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      keyword: '',
      symbols: [],
      count: entypoSet.length
    }
  },
  created() {
    const symbols = this.entypoSet.symbols
  ...
  methods: {
    query(keyword) {
      const symbols = this.entypoSet.symbols.filter((symbol) => {
